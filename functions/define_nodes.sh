define_nodes ()
{
    ...
    # For the undercloud machine I rather use virt-install because its
    # a simpler node.
    # TODO:
    # values need to be dynamic.

    eval RAM="\$${NODES[0]}"_RAM
    eval CPU="\$${NODES[0]}"_CPU
...
EOF
                ...
                do
                    mac=$(hexdump -n3 -e'/3 "52:51:0'$I'" 3/1 ":%02x"' /dev/urandom)
                    echo "$vnet=$mac" >> "$INV"
                    cat >> "$XML" <<EOF
